    ...
    ">
      ...
      ">
        ...
        <div class="flex justify-center flex-column">
          ...
          <svg xmlns="http://www.w3.org/2000/svg" width="80%" viewBox="0 0 751.57 539.42"
      ...
      ">
        ...
        <form @submit.prevent="logUserIn" class="login-form text-center space-y-5 m-5 align-center w-full"
          name="login-form">
          <input class="
            login-email
            bg-white
            ...
            px-5
            py-1
          " type="text" name="email" v-model="user.email" placeholder="Enter your email" required="true" /><br />
          <input class="
            login-password
            bg-white
            placeholder-purple-grey
            ...
            px-5
            py-1
          " type="password" name="password" v-model="user.password" placeholder="Enter your password"
            required="true" /><br />


          <div class="flex justify-center">
            <loading-buttons title="login here"
              classNm="signup-form-button bg-bright-purple rounded-xl text-sm px-5 py-2 text-cream flex justify-center"
              :isloading="isLoading" :click-func="logUserIn"></loading-buttons>
          </div>
        ...
        <span @click="GoSignup" class="signup-link md:hidden lg:hidden">New to Thriftme <b><u>Create Account
              Here</u></b></span><br />
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    async logUserIn() {
      ...
      this.isLoading = true; // Set loading state to true before requestresponse 

      try {

        const response = await axios.post("https://mevn-ecomerce-application.onrender.com/accounts/login",
          this.user,
          {
        ...
        if (response.status == 200) {
          this.$emit("loggedIn", response.data.result);
          this.$router.push(HomeVue);
        } else {
          // Handle error, e.g., display error message using toast
          this.notify(response.date.error || response.data.message || "Login failed"); // Assuming error message is in user.message

        }

      } catch (err) {
        this.notify(err.message || "network errror");
        console.log(err)
        this.isLoading = false
      }
      ...
      //   // document.cookie= `jwt=${response.cookies.jwt}; path=/`;


      //   if (user.ok) {
      //     this.$emit("loggedIn", user.result);
      //     this.$router.push(HomeVue);
      //   } else {
      //     // Handle error, e.g., display error message using toast
      //     this.notify(user.message || "Login failed"); // Assuming error message is in user.message
      ...
      //   this.notify(err.response || "network errror");
      // } finally {
      //   this.isLoading = false; // Set loading state to false after request completes (regardless of success or failure)
      // }
    },
  },
};
